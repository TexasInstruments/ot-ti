...
NLI assignments:
    NLI=0 → Thread (OpenThread)   PTY mode: HDLC-framed Spinel
    NLI=1 → BLE HCI               PTY mode: raw HCI H4
    NLI=2 → Zigbee                PTY mode: HDLC-framed Spinel
...
# ---------------------------------------------------------------------------
def read_mux_frame(ser: serial.Serial, timeout: float = 5.0) -> bytes:
    ...
    raise TimeoutError("Timed out waiting for a complete MUX frame")
...
# Uses select() so the stop_event is checked every 0.5 s.
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Re-wrap a raw Spinel payload in standalone HDLC framing.
# Used when forwarding physical -> Spinel-mode PTY so that the consumer
# (e.g. otbr-agent, wpantund) sees a standard NCP UART frame.
# ---------------------------------------------------------------------------
def spinel_to_hdlc(payload: bytes) -> bytes:
...
def _fd_read_exact(fd: int, n: int) -> bytes:
...
def read_h4_packet(fd: int) -> bytes:
    """Read one complete H4 HCI packet from file descriptor fd."""
    pkt_type = _fd_read_exact(fd, 1)[0]
    if pkt_type not in _H4_HEADER_LEN:
        raise ValueError(f"Unknown H4 packet type: {pkt_type:#04x}")

    hdr_len = _H4_HEADER_LEN[pkt_type]
    header  = _fd_read_exact(fd, hdr_len)

    len_off = _H4_LEN_OFFSET[pkt_type]
    if pkt_type == H4_ACL:
        payload_len = struct.unpack_from('<H', header, len_off)[0]
    else:
        payload_len = header[len_off]

    payload = _fd_read_exact(fd, payload_len)
...
# ---------------------------------------------------------------------------
class StackPty:
    def __init__(self, nli: int, name: str, link: str, mode: str):
        self.nli        = nli
        self.name       = name    # 'ble', 'zb', 'thread'
        self.link       = link    # symlink exposed to consumers
        self.mode       = mode    # 'hci' or 'spinel'
        self.master_fd  = -1
        self.slave_fd   = -1
        self.slave_path = ''
...
class SpinelBridge:
    ...
    def _physical_rx_loop(self, ser: serial.Serial):
        ...
        while not self._stop.is_set():
            try:
                raw = read_mux_frame(ser, timeout=1.0)
            except TimeoutError:
                continue
            ...
            if stack is None:
                self.log.debug("[SKIP] NLI=%d not configured, discarding", nli)
                continue
            ...
            try:
                if stack.mode == 'hci':
                    os.write(stack.master_fd, payload)
                else:
                    # payload is the complete HDLC frame (7e...7e) from the device's
                    # NCP layer. Write it directly — otbr-agent / wpantund expect
                    # standard HDLC-framed spinel and the framing is already present.
                    os.write(stack.master_fd, payload)
            except OSError as e:
    ...
    def _pty_rx_loop(self, ser: serial.Serial, stack: StackPty):
        ...
        HCI mode   — reads complete H4 packets, wraps in Spinel/MUX.
        Spinel mode — reads HDLC frames from PTY, strips HDLC, wraps in MUX.
        """
        self.log.info("pty-rx [%s] loop started", stack.name)
        while not self._stop.is_set():
            try:
                if stack.mode == 'hci':
                    packet = read_h4_packet(stack.master_fd)
                else:
...
def main():
    ...
    parser.add_argument('--no-zb',     action='store_true', help='Disable Zigbee PTY (NLI=2)')
    ...
    stacks = []
    if not args.no_ble:
        stacks.append(StackPty(nli=MUX_NLI_BLE,    name='ble',    link='', mode='hci'))
    if not args.no_zb:
        stacks.append(StackPty(nli=MUX_NLI_ZB,     name='zb',     link='', mode='spinel'))
    if not args.no_thread:
        stacks.append(StackPty(nli=MUX_NLI_THREAD, name='thread', link='', mode='spinel'))

    if not stacks:
        parser.error("At least one of --ble-pty / --zb-pty / --thread-pty must be specified.")
